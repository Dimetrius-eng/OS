...
&nbsp;&nbsp;&nbsp;- _Створіть змінні, що будуть містити Ваші імена та прізвища $var_name1, $var_name2, $var_name3._ 

<img src="1.png" width="730">

&nbsp;&nbsp;&nbsp;- _За допомогою команди echo виведіть імена студентів вашої команди._  

<img src="2.png" width="730">

&nbsp;&nbsp;- _Створіть псевдоніми mycal1, mycal2, mycal3 для команди cal для автоматичного виведення календарю вашого року народження._
...
&nbsp;&nbsp;&nbsp;- _Створіть функцію students_report, що порядково буде виводити спочатку імена студентів Вашої команди, а потім роки їх народження._
...
&nbsp;&nbsp;&nbsp;- _“We create such variables as $var_name1, $var_name2, $var_name3, which stored our names Name1, Name2, Name3” (у реченні спочатку виводимо назви змінних, а потім їх вміст)._    

<img src="19.png" width="730">

&nbsp;&nbsp;&nbsp;- _“We create such Aliases as mycal1, mycal2, mycal3, which can show our calendars: Calendar1, Calendar2, Calendar3”  (у реченні спочатку виводимо назву команди-псевдонімів, потім вивід цих команд)._
...
&nbsp;&nbsp;&nbsp;- _Чи можна завдання 2.1 та 2.2 ходу роботи виконати через інструкції керування без написання окремої функції, як це буде виглядати?_
...
&nbsp;&nbsp;&nbsp;- _На прикладі команди uname продемонструйте як отримати довідку. На основі отриманої додаткової інформації наведіть 5 різних варіантів виводу результату інформації по даній команді з використанням 5 різних параметрів (Options)._

Команда uname (Unix Name) використовується для виведення інформації про систему.

Отримати довідку можна наступним чином:

<img src="29.png" width="730">  
<img src="30.png" width="730">  

<br>

<br>

**П'ять варіантів виводу результату з різними параметрами:**

1. Повна інформація про систему:

<img src="31.png" width="730">  

2. Назва ядра (Kernel Name):

<img src="32.png" width="730">  

3. Мережеве ім'я вузла (Node name):

<img src="33.png" width="730">  

4. Випуск ядра (Kernel Release):

<img src="34.png" width="730">  

5. Архітектура апаратного забезпечення (Machine):

<img src="35.png" width="730">  
...
В оболонці Bash команди поділяються на чотири основні категорії: псевдоніми (Aliases), які є короткими назвами для довших інструкцій; функції (Functions), що являють собою невеликі скрипти, збережені в пам'яті; вбудовані команди (Built-in commands), які є частиною самого коду оболонки (наприклад, cd або echo); та зовнішні програми (External executables), які зберігаються у файловій системі як окремі файли (наприклад, у папках /bin або /usr/bin). Для визначення типу конкретної команди в Linux часто використовується утиліта type.

**2. Що таке змінні оточення? Які вони бувають. Як їх можна переглянути в терміналі?**

**Змінні оточення (Environment variables)** — це іменовані об'єкти, що зберігають інформацію, яка використовується оболонкою та іншими програмами для налаштування системного середовища (наприклад, шляхи до програм або ім'я користувача). Вони бувають локальними, які діють лише в поточному сеансі термінала, та глобальними (змінними оточення), які передаються всім дочірнім процесам і програмам. Переглянути всі активні змінні оточення можна за допомогою команд env або printenv, а для виводу значення конкретної змінної використовується echo $ІМ'Я_ЗМІННОЇ.

**3.** ***Опишіть змінну $PS1. Як в терміналі переглянути її вміст?**

**Змінна $PS1 (Prompt String 1)** — це одна з найважливіших змінних оточення, яка відповідає за зовнішній вигляд і вміст основного рядка запрошення (prompt) у терміналі. Вона містить набір спеціальних кодів, які вказують системі, чи потрібно відображати ім'я користувача, назву хоста, поточну директорію або час. Щоб переглянути поточне значення цієї змінної, необхідно ввести команду echo $PS1, яка зазвичай виведе рядок із символами.
...
Змінити значення $PS1 можна шляхом звичайного присвоєння нового рядка, наприклад: PS1="my-terminal> ", після чого вигляд рядка запрошення перед кожною командою миттєво оновиться на вказаний текст. Якщо змінити значення безпосередньо в терміналі, воно діятиме лише до закриття поточної сесії; щоб зробити зміну постійною (за замовчуванням), необхідно додати рядок з присвоєнням PS1 у прихований конфігураційний файл .bashrc, який знаходиться у вашій домашній директорії.

**5.** ***Для чого використовують лапки в оболонці Bash?**

Лапки в Bash використовуються для керування інтерпретацією спеціальних символів та пробілів у тексті. Подвійні лапки (`"`) дозволяють зберігати пробіли, але дозволяють системі підставляти значення змінних (наприклад, `$VAR`) та виконувати команди всередині. Одинарні лапки (`'`) повністю «заморожують» текст, ігноруючи будь-яку спеціальну роль символів усередині. Зворотні лапки (`` ` ``) або конструкція `$(...)` використовуються для виконання команди всередині іншої команди (підстановка команд).

**6.** ****Для чого використовують інструкції керування, які їх види Ви знаєте?**

Інструкції керування використовуються для зміни логіки та послідовності виконання команд, що дозволяє автоматизувати складні завдання. До основних видів належать послідовне виконання (через ;) та логічні оператори (&& — наступна команда виконується лише при успіху попередньої, та || — лише при невдачі).
...
Символ у кінці рядка запрошення вказує на рівень повноважень (привілеїв) поточного користувача в системі. Символ $ (як у записі [centos@localhost Desktop]$) означає, що ви працюєте від імені звичайного користувача, який має обмежені права доступу до системних файлів. Символ # (як у записі [root@localhost Desktop]#) свідчить про те, що ви увійшли як root (суперкористувач або адміністратор), який має необмежений доступ до всієї системи та може змінювати будь-які налаштування.

**8.** ****Яке призначення команд whereis та locate? Яка між ними відмінність?**

Команда whereis призначена для пошуку виконавчих файлів (бінарників), вихідних кодів та сторінок документації (man pages) конкретної команди, здійснюючи пошук лише у стандартних системних директоріях. Натомість команда locate шукає будь-які файли та папки по всій файловій системі, використовуючи спеціально підготовлену базу даних, що робить пошук миттєвим, проте вона може не знайти щойно створені файли, якщо база даних ще не була оновлена утилітою updatedb.

## Conclusions

&nbsp;&nbsp;&nbsp;In the course of this laboratory work, I have successfully mastered the fundamental skills of working within the Linux Command Line Interface (CLI) and the Bash shell environment. I have gained practical experience in managing shell variables (both local and environment), creating aliases for task automation, and understanding the nuances of quoting to control command interpretation.  
&nbsp;&nbsp;&nbsp;Furthermore, I have explored advanced methods of command execution using control statements such as logical operators (&&, ||) and sequential execution (;), which allow for efficient command chaining without the immediate need for complex scripting. My ability to navigate system documentation has also been enhanced through the effective use of man pages, info, and help utilities. These skills form a solid foundation for further system administration tasks and professional development in Unix-like environments.
